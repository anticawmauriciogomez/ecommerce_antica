...
export function CategoryForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success(
        category?.id ? "Categoría actualizada" : "Categoría creada",
      );
      onClose();
    } catch (error) {
  ...
  const excludeFromMenu = category?.exclude_from_menu || false;
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      <div className="space-y-6">
        ...
        <div className="flex items-center space-x-3">
          <input
            type="checkbox"
            id="exclude_from_menu"
            name="exclude_from_menu"
            defaultChecked={excludeFromMenu}
            className="h-4 w-4 rounded border-(--card-border) text-accent-gold focus:ring-accent-gold/20"
          />
          <Label
            htmlFor="exclude_from_menu"
